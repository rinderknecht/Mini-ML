...
let () =
  try
    ...
    let _bv, (fv: AST.FreeVars.t) = AST.vars ast in (* bv: bound vars; fv: free vars *)
    let () =
...
*)
  ...
  | Parser.Error ->
      ...
      and stop  = Pos.from_byte (Lexing.lexeme_end_p buffer)
      in Lexer.prerr ~kind:"Syntactical"
                     ("Parse error.", Region.make ~start ~stop);
      exit 6
